...
Databricks ClusterId and/or bearer token can be provided as parameters or retrieved from Key Vault.
...
```powershell
.\Execute-DatabricksNotebook.ps1 `
            -NotebookPath '/databricks-examples/Test-AssertEquals-DataFrames' `
...
Here is an example running a Databricks notebook with bearer token and cluster id from Key Vault.
...
. $PSScriptRoot/Databricks.ps1

If ($BearerTokenSecretName) {
    Write-Verbose "Get bearer token from Key Vault secret $KeyVaultName::$BearerTokenSecretName"
    $BearerToken = Get-AzKeyVaultSecret -VaultName $KeyVaultName -Name $BearerTokenSecretName -AsPlainText
}
...
If ($ClusterIdSecretName) {
    ...
    $ClusterId = Get-AzKeyVaultSecret -VaultName $KeyVaultName -Name $ClusterIdSecretName -AsPlainText
}
